...
function calculateAirDensity(altitude) {
  ...
  temperature =
    currentLayer.baseTemperature +
    currentLayer.lapseRate * (altitude - currentLayerBaseAltitude);
  if (currentLayer.lapseRate !== 0) {
    pressure =
      currentLayer.basePressure *
      Math.pow(
        currentLayer.baseTemperature / temperature,
        (g0 * air_molar_mass) / (currentLayer.lapseRate * R)
      );
  }
  if (currentLayer.lapseRate === 0) {
    pressure =
      currentLayer.basePressure *
      Math.exp(
        (-g0 * air_molar_mass * (altitude - currentLayerBaseAltitude)) /
          (R * currentLayer.baseTemperature)
      );
  }
...
function calculateRocketSpeedHorizontal(deltaTime) {
  ...
  let dragX =
    ...
    Cd *
    A_side *
    (velocityX - wind_velocity) *
    Math.abs(velocityX - wind_velocity);
  let netForceX = dragX;
